    ...
    async function (this: Sandbox, node: EST.UpdateExpression, scope: Scope, trace: Trace): Promise<any> {
        ...
        switch (node.argument.type) {
            ...
            case 'MemberExpression':
                ...
                const memberValue: any = object instanceof SandList
                    ? object.get(assert(member as number).to.be.number(ERROR_CODE.ONLY_NUMBER_AVAILABLE_FOR_LIST).firstValue())
                    : object.get(assert(member as string).to.be.string(ERROR_CODE.ONLY_STRING_AVAILABLE_FOR_MAP).firstValue());
